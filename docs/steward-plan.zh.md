...
> `AUTH-CP-D3` 均已关闭并消费，任何后续工作不得再援引。D2/D3 生产实现、prompt、
> campaign/model run、holdout、mock、paper/live、account、UTA、broker/demo account 连接与
> broker mutation 仍未授权；未来只能由 maintainer 分别授予新的 `AUTH-D2` / `AUTH-D3`，
> 且不自动晋级。旧 v1.1 及 P2/P3 阶段计划已归档到
...
account 连接或任何 broker mutation。下一可能动作仅是 maintainer 审查后分别签发新的
`AUTH-D2` / `AUTH-D3`；任一授权都不隐含另一项或 D4+，Wave 0.5 完成也不触发自动晋级。
...
| D2   | 补齐 autonomous execution 的确定性安全前置                     | Wave 0.5 proof 已完成；`AUTH-CP-D2` 已关闭/消费；**production runtime 未授权，须另取 `AUTH-D2`** | mandatory envelope、sizing、revoke/admission、外部 ledger commit point 的范围经独立安全审查 |
| D3   | 建立三层 eval harness                                          | Wave 0.5 proof 已完成；`AUTH-CP-D3` 已关闭/消费；**production harness/integration 未授权，须另取 `AUTH-D3`** | protocol/decision/execution 指标分离，guard containment 不计入策略得分                      |
...
任何阶段都不能因为“前一阶段没有发现 issue”自动进入下一阶段。当前没有可复用的 Wave 0.5
实现权，也没有 D2+ 的代码或运行授权；任何后续工作都必须重新取得 maintainer 明确授权。
