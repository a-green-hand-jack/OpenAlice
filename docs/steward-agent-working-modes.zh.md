...
> 版本：v0.3（2026-07-08）——记录 maintainer 最新方向：生产 steward 目标是
> **常驻 workspace session + wake envelope + decision ledger**；headless Mode 1 保留为
> campaign/评估/短任务模式，不再代表 production steward 的目标形态。具体行为契约见
...
- 本文件回答三件事：已验证过**哪种**工作方式（§2）、目标生产形态应如何收敛（§2.1）、
  有**哪些旋钮**能改它的行为（§3）、未来沿**哪些轴**扩展成更丰富的方式（§4）。

## 1. 一句话定性

当前已验证的 agent **不是 auto-trading-bot，也不是「列一个 plan 交给 broker 自动执行」**。它是一个 LLM agent，**每个决策周期像人类交易员一样现场推理一次**，用 `alice-uta` CLI 亲手下单，经 git-like 审批后由 paper broker 成交。策略即 agent（每次现推、无固定算法），broker 只负责撮合，git 审批层是可审计脊柱。

## 2. 已验证工作方式（Mode 1：逐周期自主决策 / discretionary-per-period）
...
Mode 1 的价值是**行为评估**：它证明了 v2 prompt、paper auto-push、盲化输入和 UTA
审计链能让 LLM agent 在受控战役中表现出可观察交易纪律。但它每周期都是 fresh
headless run，容易触发 coding-agent 默认反射：重读 instruction、搜索 workspace、
重新理解工具。这个模式不应直接等同于 production steward。

## 2.1 目标工作方式（Mode 1p：常驻 workspace steward / persistent-discretionary）

maintainer 方向（2026-07-08）：production steward 应该是一个**持续工作的 workspace
agent**。它可以休眠/等待事件，但不应在每个交易点重新开一个 `codex exec` 并重新理解
整个世界。

完整行为契约见 [steward-workspace-behavior-contract.zh.md](steward-workspace-behavior-contract.zh.md)；
这里仅登记它在工作方式空间里的位置：

| 维度 | Mode 1 | Mode 1p 目标 |
|---|---|---|
| 拓扑 | 单体 agent | 单体 agent |
| 进程形态 | 每周期 headless fresh run | 常驻 PTY/session，被 wake envelope 唤醒 |
| 上下文 | prompt + workspace 文件 + 工具结果 | workspace 是世界：结构化状态 + 历史 ledger + 本次 envelope + 工具结果 |
| 完成边界 | headless 进程退出 | decision ledger marker |
| 失败处理 | headless watchdog 标 interrupted | idle/stuck watchdog 唤醒；无 ledger marker 即 stuck |
| 适用 | campaign / 回测 / 短任务 | paper/live-like production steward |

这不是放松风控：每笔交易仍走 UTA git-like 审批、authz 档、guards、风险状态机；
agent 仍永远不拿 push 能力。变化只在**agent 的工作姿态和上下文流动方式**。

当前实现状态（2026-07-08）：基础 runtime seam 已落地。通用
`POST /api/workspaces/:id/sessions/:sid/wake` 可以向一个 live PTY 注入窄消息而
不 spawn 新进程；steward 专用的 `POST /api/workspaces/:id/steward/wakes` 会写
workspace-local wake/lock 文件并注入 `<STEWARD_WAKE>`；`kind: steward-wake` 的
scheduled issue 也已走同一条 persistent wake 路径。验证资产
[appendix/persistent-wake-loop-backtest](appendix/persistent-wake-loop-backtest.zh.md)
证明了多周期同 session/pid/transcript 的 wake-loop plumbing；它不声称证明 Codex
interactive TUI 的完整交易回测，Codex 的稳定完成边界仍以 decision ledger marker 为准。
...
- **目标 workspace 行为契约**在 [steward-workspace-behavior-contract.zh.md](steward-workspace-behavior-contract.zh.md)。
