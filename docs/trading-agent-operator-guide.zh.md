...
预期包含 `Logged in using ChatGPT`。Claude 必须使用 `claude.ai`
subscription OAuth，不得设置 `ANTHROPIC_API_KEY` 或 `ANTHROPIC_AUTH_TOKEN`：

```bash
test -z "${ANTHROPIC_API_KEY+x}" && test -z "${ANTHROPIC_AUTH_TOKEN+x}" && env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN claude auth status
```

预期 JSON 中 `loggedIn=true`、`authMethod="claude.ai"` 且
`subscriptionType` 非空。lab 会按 `arms[].agent` 做对应的 fail-closed
OAuth preflight，不会把 API key/token 认证算作订阅 OAuth。
...
`experiments/issue-264-isolated-mock.json`（Codex）和
`experiments/issue-266-claude-isolated-mock.json`（Claude）各自定义 isolated
evaluation 的唯一 root
...
| `arms` | 受任单元实例；`agent` 为 `codex` 或 `claude`，各自复用已有 workspace model 配置面；可选 `overlayDir` |
...
```bash
env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN node tools/campaigns/lab.mjs run ./experiments/issue-266-claude-isolated-mock.json
```
...
```bash
env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN node scripts/guardian/prod.mjs
```
...
```bash
systemd-run --user --unit=openalice-isolated-eval --collect --same-dir --property=KillMode=mixed --setenv=AQ_TEMPLATE_OVERLAY_DIR="$AQ_TEMPLATE_OVERLAY_DIR" /usr/bin/env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN "$(command -v node)" tools/campaigns/lab.mjs run ./experiments/issue-264-isolated-mock.json
```

```bash
systemd-run --user --unit=openalice-trading-agent --collect --same-dir --property=Restart=on-failure --property=RestartSec=5s --property=KillMode=mixed --setenv=AQ_TEMPLATE_OVERLAY_DIR="$AQ_TEMPLATE_OVERLAY_DIR" /usr/bin/env -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN "$(command -v node)" scripts/guardian/prod.mjs
```
...
# Claude parity run
jq '{oauth,totals,exitCode}' tools/campaigns/runs/issue-266-claude-isolated-mock/summary.json
jq '{workspace,dispatch,scheduledVerification,restartRecovery,mandate,weeks,trustworthy}' tools/campaigns/runs/issue-266-claude-isolated-mock-team-v1-claude-dev-chop-spy-r1/result.json
```
