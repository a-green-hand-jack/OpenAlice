...
- AI 可以 stage/commit 交易操作。
- 但 push 时会返回 pending ops。
- 用户需要在 Web UI 里审批。
- 审批后才会推给 broker。

当它是 `true` 时：

- AI 可以把已 commit 的操作直接 push 到 broker。
- 但 per-account `readOnly` 仍然会限制真实下单。

因此，一句“写报告并提出交易计划”本身不会自动交易。是否交易取决于该任务是否明确要求下单、交易工具是否被调用、是否 commit、是否 push，以及 `allowAiTrading` 和账户权限配置。
...
- `agent.allowAiTrading` 默认保持 `false`。
