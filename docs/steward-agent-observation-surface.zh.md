...
2. **盲化封印（`blind`，#66/#68，已合并）**：裁**市场观察**的真实性——见 §5。管的是**「能看到多真实的世界」**。

两者互不替代：授权档控执行权限，盲化控信息泄漏。

## 5. 盲化封印怎么实现（#66/#68，已合并）
...
**已修补的旁路（#68）**：E 类 `trading` 组里的市场数据工具（`searchContracts`/`getQuote`/`getContractDetails`/`expandContract`/`getMarketClock`）曾经会 fan-out 到真实 broker 账户、摸到真实市场，盲化封印没盖到它们。#68 已按同一套 `blindAllowBarSources` 白名单把这些交易行情读 runtime-gate 掉，并修掉 trim 越界；`getAccount`/`placeOrder` 等交易机制类不属于市场身份泄漏面，保持原授权档裁剪。

## 6. ANATOMY 同步纪律

- **观察面的代码真源**：`src/tool/*`（工具组）+ `src/core/workspace-tool-center.ts`（授权/盲化裁剪）+ `src/server/mcp.ts`（全局只读修剪）。[src/ANATOMY.md](../src/ANATOMY.md) 的 `tool/` 与 `workspace-tool-center.ts` 锚点已覆盖 jieke/dev 现状（授权档裁剪 + 盲化封印）。
- **盲化封印的 src/ANATOMY.md 锚点已随 #68 更新**：按 [ANATOMY 漂移规则](../ANATOMY.md)「与代码改动同 PR，不得事后补」，`sealBlindWorkspaceToolCatalog` + `BLIND_*` 常量的码侧锚点已覆盖授权/盲化裁剪。
