...
1. **授权档裁剪（`authzLevel`，jieke/dev 已在）**：按 Steward 授权档（read_only → paper → small_live → …）裁**交易能力**的可见性——`isTradingToolVisibleAtAuthzLevel`（`src/core/workspace-tool-center.ts:126-128`）、目录构建 `buildWorkspaceToolCatalog`（`src/core/workspace-tool-center.ts:155`）。全局 MCP 还把 trading 组**统一修剪为只读**（`src/server/mcp.ts:85`）。管的是**「能不能交易、到什么程度」**。
2. **盲化封印（`blind`，#66，未合并）**：裁**市场观察**的真实性——见 §5。管的是**「能看到多真实的世界」**。
...
**⚠️ 一个已知洞（待修）**：E 类 `trading` 组里的市场数据工具（`searchContracts`/`getQuote`/`getContractDetails`/`expandContract`/`getMarketClock`）会 fan-out 到**所有真实 broker 账户**、摸到真实市场，**盲化封印没盖到它们**（sonnet 对 #66 的对抗审查判为 HIGH bypass）。沙箱无真 broker 故打不穿，但架构上是真漏洞。修法：按同一套 source 白名单把这几个也 gate 掉（`getAccount`/`placeOrder` 等交易机制类不动）+ 修 trim 越界。**状态**：排队等 stress 跑完派 codex 修 → sonnet 复审 → #66 PR。

## 6. ANATOMY 同步纪律

- **观察面的代码真源**：`src/tool/*`（工具组）+ `src/core/workspace-tool-center.ts`（授权/盲化裁剪）+ `src/server/mcp.ts`（全局只读修剪）。[src/ANATOMY.md](../src/ANATOMY.md) 的 `tool/` 与 `workspace-tool-center.ts` 锚点已覆盖 jieke/dev 现状（授权档裁剪），**尚不含盲化封印——因 #66 未合并，符合预期**。
- **盲化封印的 src/ANATOMY.md 锚点更新属于 #66 PR**：按 [ANATOMY 漂移规则](../ANATOMY.md)「与代码改动同 PR，不得事后补」，`sealBlindWorkspaceToolCatalog` + `BLIND_*` 常量的码侧锚点在 #66 落地时随代码一并加进 src/ANATOMY.md（已列入 #66 修复任务范围）。本文件先登记设计与三态语义。
