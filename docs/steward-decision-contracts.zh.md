...
过期的、或本轮 intent 同标的的 thesis，必须在 `thesisDispositions` 中恰好出现一次；
`disposition: keep` 用于显式保持（未过期、非同标的的 thesis 可省略，视为隐式 keep）。
散文解释留在 completion，属人读辅助，不进 validator 信任链。
