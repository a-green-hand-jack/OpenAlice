...
| **v9-RUNTIME** | ledger v3 / Snapshot M1 机械契约同步 | **`src/workspaces/templates/steward/files/instruction.md`**（Wake Loop + Decision Ledger Shape）+ launcher-owned per-wake wake message + 生成的 `.alice/steward/validate-ledger.mjs` | 同左；per-wake message 确保旧 persistent session 每轮都收到当前契约 | **AUTH-D2 issue #174 已授权实现；未授权任何运行/campaign/paper/live** | 只同步已批准 decision contract v0.3：新写入为 ledger v3，decision 改为 `no_trade | propose_change | reduce_risk | blocked`，新增结构化 `intent`、按 `(wakeId, instrument)` 标识的 `thesisDispositions`，并要求 intent 复制 wake-bound Snapshot M1 的 id/hash。删除 v2 prompt 中“agent 直接写数量并执行 broker mutation”的旧契约，明确本阶段 agent 只写 Decision Intent、`actions: []`/`pendingHash: null`；数量、授权和 mutation 仍归确定性层。此条是契约接线，不新增参与度、仓位目标、风险压力或模型行为调优；D4+ 运行仍封存。 |
...
> 70–85% winner exposure target 和 per-wake add evaluation 加入 Participation Bias。
> Canonical matrix 虽 10/10 audit-clean，但 performance 仍为 7/8 gateable，且 review 发现
> 固定 70–85% 目标在**没有** `max-position-size` guard 的账户上可能直接形成过度集中；测试中
> 至少三个 70% 级意图是被专用 60% guard 拒掉才没有执行。Maintainer 因此只合并实验记录与
> 架构证据，**不把该候选激活到默认 instruction**。完整记录见 §17 与
...
| Steward 唤醒指令（实质内容，当前 **v9-RUNTIME**） | `src/workspaces/templates/steward/files/instruction.md` 全文（321 行）：World Boundary `7-22`、Mandate `24-41`、Evidence-First Reasoning `43-62`、Participation Bias `64-111`、Risk Discipline `113-133`、Wake Loop `135-210`（8 步；v3 Decision Intent、Snapshot M1、validator-only writer）、Decision Ledger Shape `212-312`（`version: 3`、`propose_change`/`reduce_risk`、`intent`、复合 `thesisDispositions`）、Safety `314-321`；另有 `src/workspaces/steward/injector.ts:5-17` 的每轮 wake 消息覆盖旧 persistent-session 记忆 | steward workspace 当前完整行为与 ledger 写入契约。v3-v7 的推理/风控实质仍见 §7、§10-§13；v8-RUNTIME 历史契约见 §15；当前 v9-RUNTIME 只做 D1 已批准契约的机械同步，禁止数量和 broker mutation，未授权 campaign/paper/live |
...
## 17. 未启用的 #126 participation-policy 实验

Issue #126 起于 v7 NVDA 的稳定 under-participation：三次 isolated run 为 +17.7% / +17.4% /
+16.4%，都低于 +25% gate。候选 commit `7a3b8d52` 试了三条 prompt lever：首 wake 有趋势证据
时默认开首仓；赢家目标敞口 70–85% equity 或靠近 guard；趋势仍有效且未到目标时每 wake 默认
评估加仓。

在 exact tested HEAD `e27efdb653e09a19c005d20a867638d146695927` 上，`v8matrix7` 完成
10/10 audit-clean、60/60 wakes。它强力验收了 #134/#136/#137/#139/#140 的 runtime
integrity，但 behavior verdict 仍与 v7 相同：排除两个 guard-infeasible bull cell 后 7/8，
NVDA +20.7% 仍未过 +25%。完整矩阵见
[appendix/steward-v8-candidate-20260711.md](appendix/steward-v8-candidate-20260711.md)。

合并前独立 review 找到 blocker：默认账户 `guards` 可以为空，UTA 不会隐式创建
`max-position-size`；而候选把 70–85% 当成一般目标并把 guard 当 sizing 边界。Canonical evidence
里 `bull-cx`、`dev-bull-3690hk`、`dev-chop-spy` 都出现约 70% 的加仓意图，靠测试专用 60%
guard 的 `policy_denied` 才没有执行。故“最终 drawdown/分类通过”只能证明 deterministic guard
兜底有效，不能证明候选在无 guard 账户安全，也不能证明 chop 防守没有产生危险意图。

最终决策：默认 `instruction.md` 保留 `jieke/dev` 现有参与策略；#126 候选只保存在 branch、
commit、issue 评论和本节，不随 consolidation PR 激活。若未来重开，必须先定义**无 guard 的
独立保守上限**、确认具体 `max-position-size` 的语义与 headroom，并覆盖 no-guard / low-guard /
不同 guard threshold 的行为验证。Holdout/live 继续封存。
