...
timeout、账户残留；#137 的 `cashQty` 还在真实 cell 中产生了非零 fractional fill。
...
bear/chop 的最终 verdict 继续通过，但至少三个约 70% exposure intent 是被测试账户的 60%
guard 拒掉才没有执行，因此不能外推到无 max-position guard 的账户。独立 review 决定只把
