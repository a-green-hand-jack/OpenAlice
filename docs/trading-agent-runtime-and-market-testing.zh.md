...
flowchart LR
  ...
  Session --> CLI[alice-uta / MCP tools]
...
| 持久交易状态 | `.alice/steward/*.json` 和 ledger | OpenAlice + agent | 读取账户绑定、授权档、历史决策、成本状态 | `config.json`、`context-manifest.json`、`ledger/decisions.jsonl` |
| 本次任务边界 | wake envelope JSON | selector / supervisor | 知道本轮为什么醒来、看哪个账户、deadline 是什么、有哪些市场上下文 | `wakeId`、`accountId`、`marketContext.bars` |
| 实时账户和市场事实 | `alice-uta` / MCP 工具结果 | UTA / market-data layer | 跑 checklist，查账户、持仓、订单、risk、quote、bar source | account info、positions、riskState、market clock |
...
```mermaid
flowchart TB
  Agent[Trading agent] --> Decision[Decision ledger JSONL]
...
| 输出类别 | 形式 | 权威含义 |
| --- | --- | --- |
| 本轮完成标记 | `.alice/steward/ledger/decisions.jsonl` 一行 JSON | supervisor 用它判断 wake 是否完成；这是单次 wake 的完成边界 |
...
sequenceDiagram
  ...
  alt no trade
    A->>W: append ledger decision=no_trade
  else propose trade
    ...
    U-->>A: autoPush result
    A->>W: append ledger with actual action and pendingHash
  else blocked
    A->>W: append ledger decision=blocked with reason
  end
  S->>W: read ledger marker
...
当前 checked-in 的 4 个 cells 确实全部来自 crypto：
...
这解释了“看起来只有虚拟货币”的问题：不是架构只能测虚拟货币，而是当前实验集确实只放了
crypto cells，而且使用 mock-simulator 执行。
