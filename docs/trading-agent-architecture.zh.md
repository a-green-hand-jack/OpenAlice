...
> **#264 当前边界。** 受任单元是实现无关的委任契约，不等同于 LLM；当前 v1 实例使用
> Codex。Trading Team 独占 policy/strategy，OpenAlice instruction 只承载 lifecycle、wake、
> checklist、ledger、recovery 和 safety mechanics，UTA 独占 Risk Envelope、执行与 broker。
> finite evaluation 的当前路径是 `lab -> run-cell`。旧 prompt/策略描述仅为历史背景；当前
> 行为和操作入口见 [v0.5 Behavior Contract](steward-workspace-behavior-contract.zh.md) 与
> [Operator Guide](trading-agent-operator-guide.zh.md)。
>
> 状态：#264 当前边界摘要与历史实现说明（原基线 `jieke/dev`，2026-07-11）。当前事实以代码、
> v0.5 行为契约和 operator guide 为准；正文保留的旧实验/Plan v2 内容不构成现行授权。本文解释 trading-agent、
...
受任单元不是 OpenAlice 内部一个“传入参数、返回结果”的函数，也不是 Codex/Claude 的
同义词。当前 v1 交易受任单元住在独立 workspace 中，并使用 Codex 原生 CLI 持久会话实现。
...
### 2.1 Entrusted unit / 当前 Codex 实例

- **Entrusted unit** 是接受 mandate、资本/额度、Risk Envelope、scope、有效期、身份和心跳
  义务，并从受限观察产生带身份 Trade Intent 的任意决策者。
- 当前 v1 恰好用 Codex 作为 Team 提供的受任单元实现；Codex 的模型循环和 transcript 不构成
  通用契约。
- Team policy 决定策略；OpenAlice instruction 只定义 mechanics。受任单元不拥有账户真相、
  风险许可或最终执行权。
...
wrapper prompt、instruction/skill 的路径与 SHA-256、schema 版本。当前 manifest 声明
`decisionLedger: 3` 并引用 ledger v3 schema artifact，与 validator/writer 一致。它不包含行情、
指标、新闻或账户快照；市场事实来自 wake envelope、campaign 文件或实时工具查询。
...
| `mandate`          | 可选 v1 root mandate；绑定 mandate/受任单元身份、资本、scope、有效期、心跳与 Risk Envelope   |
...
| `machine_thread_account_mismatch`  | stored thread 属于另一 accountId，弃用并起 fresh thread（issue #155）|
...
- **provider 记入 thread store，跨 provider resume 有护栏**：thread store 记 `provider`，stored
  thread 与本轮 provider 不符时不 resume（见 `machine_thread_provider_mismatch`）；同理 thread
  store 也记 `accountId`（issue #155），stored thread 属于另一 accountId 时同样不 resume（见
  `machine_thread_account_mismatch`）——没有 `accountId` 的旧记录视为 legacy，下一次写入即采纳
  当轮 accountId，不算 mismatch。
...
手编 JSON。`readStewardConfig` 读取时会对照 `stewardConfigSchema` 做一次**观察性**校验——
识别出的字段值不合法（如 `controlFace: 'PTY'`）时打一条 `steward.config_invalid` 结构化
warning，列出全部违规 key/value，但从不阻塞读取、不改写文件；下方判定函数仍是唯一的
enforcement 点，自行收紧，issue #153）：
...
spec 断言其等于已安装包版本，静默 bump 直接 CI 失败。系统 `codex` 二进制独立于本仓库升级
（issue #152）：driver 初始化时 fire-and-forget 探测一次 `codex --version`（按 bin 路径进程级
memoize，绝不加派发延迟），与 pin 值不符或探测失败（二进制缺失/输出不可解析）都只打
`steward.codex_version_mismatch` / `steward.codex_version_probe_failed` warning，从不阻塞 wake。
...
native Write/Edit
  ...
  -> strict v3 schema + wake/mandate/Trade Intent identity + prior integrity checks
...
## 11. 历史冻结：Plan v2 方向（已被 #264 取代）

> 本节只保存 2026-07-11 的 Plan v2 设计与实验语境，已被 issue #264 的 v0.5 契约取代；
> 不授权新增 sizing/runtime、prompt 实验、holdout、paper 或 live。下文“当前”“未来”等词均指
> 当时计划，不是现行路线。
...
当时的 `context-manifest.json` 记录 instruction/skill/schema 的版本，不是 agent 决策时的信息集。
...
当时计划要求 control/tool/state protocol 进入 freeze。只有出现违反既有不变量的新证据，才重开 runtime
...
现行边界与操作入口以 [v0.5 Behavior Contract](steward-workspace-behavior-contract.zh.md) 和
[Operator Guide](trading-agent-operator-guide.zh.md) 为准；
[steward-plan.zh.md](steward-plan.zh.md) 仅保留历史冻结证据。
