...
**issue #134 新增（ledger 审计完整性 / receipt reconciliation）**：常驻 Codex
session 曾在一次运行里先写入 week-1 ledger entry（supervisor 已据此标 `done`），随后
`Delete File` 整个 `decisions.jsonl` 并从 week-2 重建，导致 terminal wake 仍是 `done`
但它的首胜 ledger entry 消失，而当时只校验当前 wake 的 validator 照样通过。修复引入
**ledger receipt**。

> **诚实边界（不要过度宣称）**：这是**corruption-evident（可发现意外损坏/漂移）**，
> **不是 tamper-proof/adversarial**。receipt、wake record、ledger 同处一个 agent
> 可写的信任域，协同改写三者仍可绕过；它抓的是**意外**的完整性漂移——正是触发本
> issue 的那种「session 删了又重建 decisions.jsonl，丢掉一个已完成 wake」。

- **首次终态即留证**：supervisor 第一次把某*已派发*的 ledger-backed wake（`done`/
  `blocked`/`error`）推进到终态时，同一次 `updateStatus` 写入 `wake.ledgerReceipt`
  ——记录首胜 entry 的 **canonical 语义 SHA-256 指纹**
  （`src/workspaces/steward/ledger-receipt.ts` 的 `canonicalDecisionFingerprint`：
  只取已知 schema 字段做**语义投影**、丢弃未知顶层键，再按 key 递归排序后 hash，所以
  纯格式/空白/加无关键都不误报，语义改动才会）加上 wakeId/status/decision/at/accountId。
  receipt **只写一次、不覆盖**。一个 golden 向量在 TS 与生成的 JS validator 两侧被同一
  常量断言，保证两处指纹永不漂移。
- **未派发不算 ledger-backed**：POST 阶段 session 选择/注入失败会把 wake 标成终态但
  **无 `injectedAt`、无 receipt**（注入失败现已按 supervisor 语义映射为 `stuck`）。这类
  从未运行的 wake **不参与**完整性复核/validator/campaign 审计，否则会变成永久误报。
- **每 tick 单次解析 + 复核所有终态 wake**：`tick()` 先 `ledgerStore.readIndex()`
  **一次性**解析整份 ledger（首胜索引 + 有效 entries + invalid + duplicates），供 active
  推进、终态复核、诊断、成本共用（消除按 wake 逐次 readFile 的 O(n²)）。首胜行的选择在
  TS 与 JS 两侧一致：**同一 wakeId 的第一条 JSON-可解析对象行**（即使 schema 非法也算首胜，
  只是不驱动状态推进、并被列入 invalid）。对每个*已派发的* ledger-backed terminal wake，
  用当前首胜 entry 与 receipt 比对：entry 消失→`entry_missing`；指纹不符→`entry_mutated`；
  无 receipt 的历史 wake 若还有 entry 则一次性 `bootstrapped` 补录、若 entry 也没了则
  `entry_missing_no_receipt`（诚实上报，绝不伪造）。
- **事件去重 + 恢复**：违规通过 `wake.ledgerIntegrity` 去重——相同
  `(kind, expected/actual 指纹)` 只写**一条** `ledger_integrity_violation` 事件（每 tick
  仍在 `warnings[]` 里可见，但 `supervisor.jsonl` 不无界增长）；entry 恢复后清除标记并写
  `ledger_integrity_recovered`。绝不悄悄改写历史，也绝不让后来的重复 entry 顶替首胜
  entry（延续 #125 D3 first-wins）。
- **bootstrap 的历史局限**：pre-#134 的终态 wake 没有原始 receipt，只能从**当前** entry
  一次性补录（`bootstrapped: true`）——即对该 wake 的检测**从这一刻起**才生效，当前 entry
  被信任一次；这一点在 receipt 语义与文档里明示，不假装拥有历史原件。
- **生成的 workspace validator 交叉核对**：`templates/steward/bootstrap.mjs` 生成的
  `validate-ledger.mjs` 除了校验当前 wake，还遍历 `.alice/steward/wakes/` 下所有*已派发的*
  terminal（`done`/`blocked`/`error`）wake record，要求它们的首胜 ledger entry 仍在、
  且（带 receipt 时）指纹一致——语义投影与指纹算法与 `ledger-receipt.ts` 逐字节一致
  （golden 向量守护）。timeout/stuck、以及未派发的 wake 跳过。
- **campaign finalization 信任判定**：`tools/campaigns/run-cell.mjs` 收尾时用
  `finalizationTrust`（`tools/campaigns/_lib.mjs`）：ledger-backed terminal wake 集合必须
  等于导出 ledger 的首胜 wakeId 集合，且 supervisor 未记录任何
  `ledger_integrity_violation`，否则 `result.trustworthy=false` 并以非零码退出；报告渲染
  会把不可信 run 显式标 `AUDIT-INVALID`，不能被当成正常 PASS 消费。
  **注意两种检测的定位不同**：campaign 的 `weeks[]` 是**workspace 之外的见证**（harness
  在每周轮询时抓到的终态快照，agent 改不到），而生产环境的 receipt 是**workspace 之内**的
  意外损坏检测——两者互补，前者是外部对照，后者是常驻自检。

**issue #136 新增（write-then-validate finalize barrier）**：#134 上线后的首个 clean
canary 暴露一个确定性竞态——agent 写下 schema-valid 的 ledger 草稿后，supervisor **立即**
把 wake 终态化并抓 receipt（fp1）；随后 agent 按 #125 允许**原地更正**了同一行（fp2），
再跑 validate-ledger；#134 正确地把这次「终态后 fp 改变」判为 `entry_mutated`，使整个
run `trustworthy:false`。根因是 raw 写入与 supervisor 终态化之间**缺少 finalize 屏障**。
修复引入 **finalization marker（验证即提交点）**：

- **raw 存在不再等于终态**：每个新 wake 在 `create()` 时带 `finalizeProtocol: 'marker'`；
  supervisor 的 active 分支对这类 wake 只有在**同时**满足「存在可解析首胜 entry」且「存在
  finalize marker 且其指纹等于当前首胜语义指纹」时才终态化并抓 receipt。marker 缺失＝尚未
  验证（静默等待）；marker 指纹不符＝验证后又改了行（等待 + 一条清晰 warning，提示重跑
  validate-ledger）。
- **validator 是提交点**：生成的 `validate-ledger.mjs` 在**通过全部检查后**
  （当前 entry strict v2、无重复、所有前序 terminal wake 完整性/指纹）**原子发布**
  `.alice/steward/finalize/<encodeURIComponent(wakeId)>.json`——`{version, wakeId,
  fingerprint(语义指纹), validatedAt, schemaVersion}`，tmp+rename 原子写、编码路径对任意
  wakeId 安全。任一检查失败会提前 `exit(1)`，**不写 marker**（schema 失败、前序 terminal
  完整性失败都不会发布当前 wake 的 marker）。
- **合法的同 wake 更正**：更正后重跑 validator 会**原子替换**旧 marker（指纹更新到 fp2）；
  下一次 tick 才终态化并抓**更正后**的 receipt（fp2）——原始竞态的假阳性由此消失。
- **终态后不变**：一旦抓了 receipt，#134 的 missing/mutated 检测与 first-wins 语义完全不变
  （终态后的改写仍是真·corruption violation）。
- **向后兼容（有界规则）**：升级前已在飞行、无 `finalizeProtocol` 的**遗留** active wake
  仍按旧「raw 存在即终态」路径完成（避免它们永久卡住）；但**每个新 wake 都要求 marker**，
  不是「以后所有 v2 raw 行永远无屏障终态化」。已终态的 wake / receipt 继续走 #134
  reconcile/bootstrap；timeout/stuck/未派发不需要 marker。
- **prompt 半边**：`files/instruction.md` 完成协议明确「运行 validator 才是提交点；对 ledger
  行的任何后续编辑都必须重跑 validator，否则 marker 与行不符、wake 不会完成」（见 prompt
  anatomy 台账 §3 v4/v8 组件的 #136 增补）。

**issue #139 新增（wakeId 身份绑定，禁止冒充 active wake）**：#136 上线后的 canary
（`v8matrix5-dev-bull-3690hk` week 3）暴露：steward 把**前一个 wake 的 UUID 尾段**误抄进
本次 ledger 顶层 `wakeId`（`d2477474-...-8508...`，一个从未 post 过的混合 id），而
`completion.evidenceRefs` 的 `wake:` 自引用仍是**正确**的 active wake（`...34f...`）。旧
validator 仅按「agent 传入的 argv id == 顶层 wakeId」找到该条并通过 schema，为这个幽灵 id
写了 finalize marker；真正的 active wake 等不到 entry/marker，跑满 602s timeout；campaign
audit 正确判 `trustworthy:false`。根因是**验证/终态从不把 entry 绑定到外部已知的 active
wake 身份**，只信 entry 自报的 id。修复三层绑定：

- **entry 自洽（服务端 schema + 生成 validator 都强制）**：v2 entry 的
  `completion.evidenceRefs` 必须恰好包含 `wake:<顶层 wakeId>` 自引用；出现指向**别的** wake
  的 `wake:` 引用（复制/矛盾 id）或缺失自引用即 invalid。这一条本身就能抓住本 bug（顶层
  `...8508...` 与 evidence `...34f...` 不符）。
- **外部绑定（validator）**：生成的 `validate-ledger.mjs` 在校验前先读
  `.alice/steward/wakes/<encodeURIComponent(wakeId)>.json`——wake record 由**服务端** post
  时写入，是外部真源；要求它**存在且非终态**。幽灵/typo id（无 wake record）或已终态 wake
  （replay）都直接 fail、不写 marker。因此 marker 只会绑定到一个真实的 active posted wake。
  跨进程重启/恢复安全（wake record 持久化）、多 workspace 安全（只读本 workspace 的 wakes/）、
  多 pending wake 安全（按具体 wakeId 读各自文件）。
- **主动可纠正反馈（supervisor）**：单次解析的 ledger index 现额外收集
  `identityMismatches`——顶层 wakeId 与某条 `wake:` 自引用不一致的行（从 raw obj 采集，
  与 schema 有效性无关）。supervisor 对每个 active 非终态 wake A，若存在「顶层是别的 id 但
  evidence 自引用 A」的错填 entry，就发一条 **actionable** `ledger_identity_mismatch`
  事件（按 `(wake, 错填 id)` 用 `wake.ledgerIntegrity` 去重，只发一次，每 tick 仍在
  `warnings[]` 可见）并提示把顶层 wakeId 改成 A 后重跑 validator——**不再静默等满 timeout**。
  它**从不**据此终态化（绝不把写入中的中间态误判为完成）；entry 改对后 mismatch 解除、
  marker 清除并发 `ledger_identity_recovered`；wake 正常终态时该 pre-terminal 标记一并清掉，
  交回 #134 receipt 复核。
