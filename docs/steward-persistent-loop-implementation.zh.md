...
第一版不立刻改 schedule scanner。先通过手动 API 做 smoke：
...
scanner 遇到 `kind: steward-wake` 时走 `/steward/wakes` 的内部 service seam，而不是
`dispatchHeadlessTask`。普通 issue 仍走 headless，不回归现有自动化。

## 10. Smoke 验收

第一轮实现完成后，先跑 smoke，不跑收益实验：

1. 创建/选择一个 steward workspace，启用 Codex。
...
smoke 通过后，再做行为实验。此处仍不以收益为目标，而验证工作方式：
...
1. **PR A：schema + workspace file stores**
   - `types.ts`、`wake-store.ts`、`ledger-store.ts`、单元测试。
2. **PR B：PTY input seam**
   - `PersistentSession.writeInput`、`SessionPool.writeToSession`、测试。
3. **PR C：steward wake API + injector**
   - `POST /steward/wakes`、session reuse/spawn、wake file、manual smoke。
4. **PR D：supervisor + lock + cost**
   - timeout/stuck/done detection、lock file、cost summary。
5. **PR E：schedule integration**
   - scheduled issue `kind: steward-wake` 走 persistent wake，不走 headless。
6. **PR F：steward template/instructions**
   - built-in `steward` template，版本化 wrapper prompt / skills / ledger instructions。

如果实现中发现 PR F 必须前置，可以把最小 template 放到 PR C，但不要把复杂工作流提前。

## 13. 待批注问题

1. 第一版是否接受“普通 PTY stdin + Enter”作为 Codex interactive wake 注入 seam？
2. `.alice/steward/` 是否应进入 workspace git 跟踪？我的倾向：ledger/config/README 跟踪，
   lock/runtime status 可不跟踪或用 git exclude。
3. 成本策略第一版是只记录，还是超过月度预算直接阻止新 wake？
4. schedule integration 是否放到第一批实现，还是 smoke 先只保留手动 API？
5. steward template 是否必须先建，还是先用现有 chat template + 注入 config 试跑？
