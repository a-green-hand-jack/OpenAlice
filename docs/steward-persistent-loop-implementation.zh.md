...
- **首次终态即留证**：supervisor 第一次把某 ledger-backed wake（`done`/`blocked`/
  `error`）推进到终态时，同一次 `updateStatus` 写入 `wake.ledgerReceipt`——记录首胜
  entry 的 **canonical-JSON SHA-256 指纹**（`src/workspaces/steward/ledger-receipt.ts`
  的 `canonicalDecisionFingerprint`：对 parse 后的对象按 key 递归排序再 hash，所以
  纯格式/空白改写不会误报，语义改动才会）加上 wakeId/status/decision/at/accountId。
  receipt **只写一次、不可覆盖**。
- **每 tick 复核所有终态 wake**：`tick()` 不再跳过 terminal wake；对每个 ledger-backed
  terminal wake 用当前首胜 entry 与 receipt 比对。entry 消失→`entry_missing`；指纹不符
  →`entry_mutated`；无 receipt 的历史 wake 若还有 entry 则一次性 `bootstrapped` 补录、
  若 entry 也没了则 `entry_missing_no_receipt`（诚实上报，绝不伪造）。任何违规写一条
  `type: "ledger_integrity_violation"` 事件并进 `warnings[]`，绝不悄悄改写历史，也绝不
  让后来的重复 entry 顶替首胜 entry（延续 #125 D3 first-wins）。
- **生成的 workspace validator 交叉核对**：`templates/steward/bootstrap.mjs` 生成的
  `validate-ledger.mjs` 除了校验当前 wake，还遍历 `.alice/steward/wakes/` 下所有
  terminal（`done`/`blocked`/`error`）wake record，要求它们的首胜 ledger entry 仍在、
  且（带 receipt 时）指纹一致——所以只要之前某个完成决策从 ledger 消失或被改写，校验
  当前 wake 就会失败。指纹算法与 `ledger-receipt.ts` 逐字节一致。timeout/stuck 不需要
  ledger entry，跳过。
- **campaign finalization 审计**：`tools/campaigns/run-cell.mjs` 收尾时用
  `auditFinalization`（`tools/campaigns/_lib.mjs`）做集合相等校验——ledger-backed
  terminal wake 集合必须等于导出 ledger 的首胜 wakeId 集合，发散即把 `result.audit.valid`
  置 false 并以非零码退出，不再写出「六个 done 却只导出五条决策」的可信结果。
