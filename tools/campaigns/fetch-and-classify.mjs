 ...
 *
 * Equity (US / HK / SG) is pulled via yfinance through `uv run --with yfinance`
 * (yfinance is NOT pre-installed); crypto via Binance public klines.
 *
 * Usage:
...
function parseArgs(argv) {
  ...
  if (!['crypto', 'equity'].includes(out.asset)) throw new Error('--asset must be crypto|equity');
...
function printHelp() {
  ...
  --symbol S     ticker (equity: NVDA, 0700.HK, D05.SI) or pair (crypto: BTCUSDT)
  --asset A      crypto | equity            (default crypto)
...
/** Equity daily bars via yfinance (uv-managed, throwaway env). Newest-last. */
